...
app.post("/api/match", upload.fields([{ name: "resume" }, { name: "jd" }]), async (req, res) => {
  try {
    ...
    const resumeText = (await pdfParse(fs.readFileSync(resumePath))).text.toLowerCase();
    const jdText = (await pdfParse(fs.readFileSync(jdPath))).text.toLowerCase();
    ...
    for (let skill of skillsList) {
      ...
      if (jdText.includes(lower)) {
        if (resumeText.includes(lower)) matchedSkills.push(skill);
    ...
    const prompt = `
You are a job application assistant. Given the job description and resume below, analyze how well the resume matches the job. Return the response in the following JSON format:
...
"""`;
    ...
    res.json({ matchScore, matchedSkills, missingSkills, feedback });
  } catch (err) {
